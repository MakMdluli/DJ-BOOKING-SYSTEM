  ...
  try {
    ...
    if (event === 'payment_requested' || event === 'booking_declined' || event === 'booking_cancelled') {
      ...
      if (event === 'payment_requested') {
        ...
        const link = `${root}/pay.html?token=${encodeURIComponent(b.payment_token)}`;
        const documentsLink = `${root}/documents.html?token=${encodeURIComponent(b.payment_token)}`;
        subject = 'Your Scorpion_Jr booking has been accepted — payment required';
        title = 'Booking accepted — deposit required';
        body = `<p>Your booking request has been accepted.</p><p>${bookingSummary(b)}</p><p><strong>Deposit required: E ${money(b.deposit_amount)}</strong></p><p>Your booking is only confirmed after the deposit has been verified.</p><p><a href="${esc(link)}">Open your payment page</a></p><p><a href="${esc(documentsLink)}">View / download your quotation</a></p>`;
